import * as React from "react";
import Svg, { Path } from "react-native-svg";
const SVGComponent = ({width, height ,...props}: any) => (
  <Svg
    width={width}
    height={height}
    viewBox="0 0 22 22"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    {...props}
  >
    <Path
      d="M16.5 1H7C4.79086 1 3 2.79086 3 5V17C3 19.2091 4.79086 21 7 21H16.5C18.7091 21 20.5 19.2091 20.5 17V5C20.5 2.79086 18.7091 1 16.5 1Z"
      stroke="#4B4B4B"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
    <Path
      d="M9.59 12.74C8.961 13.162 7.313 14.022 8.317 15.098C8.807 15.624 9.354 16 10.04 16H13.96C14.646 16 15.193 15.624 15.683 15.098C16.687 14.022 15.039 13.162 14.41 12.741C13.699 12.2582 12.8594 12.0001 12 12.0001C11.1406 12.0001 10.301 12.2582 9.59 12.741M4 5H1.5M4 11H1.5M4 17H1.5M14 8C14 8.53043 13.7893 9.03914 13.4142 9.41421C13.0391 9.78929 12.5304 10 12 10C11.4696 10 10.9609 9.78929 10.5858 9.41421C10.2107 9.03914 10 8.53043 10 8C10 7.46957 10.2107 6.96086 10.5858 6.58579C10.9609 6.21071 11.4696 6 12 6C12.5304 6 13.0391 6.21071 13.4142 6.58579C13.7893 6.96086 14 7.46957 14 8Z"
      stroke="#4B4B4B"
      strokeWidth={1.5}
      strokeLinecap="round"
      strokeLinejoin="round"
    />
  </Svg>
);
export default SVGComponent;
